...
data <- read.csv(file="final_mean_jumps.csv",header=FALSE);

experiment <- c(as.numeric(data[,1]));
alpha <- c(as.numeric(data[,2]));
beta <- c(as.numeric(data[,3]));
jumps <- c(as.numeric(data[,4]));

result <- data.frame(alpha, beta, jumps);

png(file=paste("mean_jumps.png"),width=1280, height=1280);
meanJumpPlot <- wireframe(result$jumps ~ result$alpha * result$beta, data=result,
